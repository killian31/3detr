...
def data_viz(data_dir, dump_dir=os.path.join(BASE_DIR, "data_viz_dump")):
    ...
    idxs = np.array(range(1, nb_data + 1))
    np.random.seed(0)
    np.random.shuffle(idxs)
    # for idx in range(len(sunrgbd)):
    for idx in range(nb_data):
        data_idx = idxs[idx]
...
def extract_sunrgbd_data(
    ...
    """Extract scene point clouds and bounding boxes with class-based stopping criteria."""
    ...
    # Dictionary to track occurrences of each class
    class_counts = {cls: 0 for cls in type_whitelist}

    for data_idx in data_idx_list:
        print("------------- ", data_idx)
        objects = dataset.get_label_objects(data_idx)

        # Skip scenes with 0 object or all classes are already saturated
        ...
        if all(class_counts[cls] >= max_counts for cls in type_whitelist):
            print("All classes reached required count. Stopping data processing.")
            break
        ...
        for obj in objects:
            ...
            # Stop adding new samples if this class already has enough data
            if class_counts[obj.classname] >= max_counts:
                continue
        ...
        if not added_to_dataset:
            continue

        obbs = np.vstack(object_list) if len(object_list) > 0 else np.zeros((0, 8))
...
if __name__ == "__main__":
    ...
    if args.gen_v2_data:
        extract_sunrgbd_data(
            os.path.join(BASE_DIR, "sunrgbd_trainval/train_data_idx.txt"),
            split="training",
            output_folder=os.path.join(BASE_DIR, "sunrgbd_pc_bbox_votes_50k_v2_subsample_train"),
            save_votes=True,
            num_point=50000,
            use_v1=False,
            skip_empty_scene=False,
        ...
        extract_sunrgbd_data(
            os.path.join(BASE_DIR, "sunrgbd_trainval/val_data_idx.txt"),
            split="training",
            output_folder=os.path.join(BASE_DIR, "sunrgbd_pc_bbox_votes_50k_v2_subsample_val"),
            save_votes=True,
            num_point=50000,
            use_v1=False,
            skip_empty_scene=False,
