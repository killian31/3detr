...
def generalized_box3d_iou_tensor_non_diff(
    ...
    if batch_intersect is None:
        ...
        ymax = torch.min(
            corners1[:, :, 0, 1][:, :, None], corners2[:, :, 0, 1][:, None, :]
        )
        ymin = torch.max(
            corners1[:, :, 4, 1][:, :, None], corners2[:, :, 4, 1][:, None, :]
        )
...
def generalized_box3d_iou_convex_hull_nondiff_tensor(
    ...
    inter_vols = generalized_box3d_iou_tensor_jit(
        corners1, corners2, nums_k2, rotated_boxes, return_inter_vols_only=True
    )
    ...
    if rotated_boxes:
        ...
        enclosing_vols = enclosing_box3d_convex_hull_numba(
            corners1_np, corners2_np, nums_k2, mask, enclosing_vols_np
        )
