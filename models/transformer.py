...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        self, encoder_layer, num_layers, norm=None, weight_init_name="xavier_uniform"
    ):
        super().__init__()
...
class MaskedTransformerEncoder(TransformerEncoder):
    def __init__(
        ...
        super().__init__(
            encoder_layer, num_layers, norm=norm, weight_init_name=weight_init_name
        )
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.masking_radius[idx] > 0:
                mask, xyz_dist = self.compute_mask(
    ...
    def forward(
        ...
        if self.normalize_before:
            return self.forward_pre(
                src, src_mask, src_key_padding_mask, pos, return_attn_weights
            )
